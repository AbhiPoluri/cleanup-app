...
internal sealed class WhiteboardRemote : IAsyncDisposable
{
    ...
    private async Task Events(HttpContext c)
    {
        ...
        try
        {
            ...
            foreach (var m in msgs) await WriteEvent(c, JsonSerializer.Serialize(new { type = "msg", id = m.Id, role = m.Role, text = m.Text, image = m.Image is int n ? $"/img/{n}?t={_token}" : null }));
        ...
        string status, bool running, bool muted)
    {
        ...
        foreach (var m in messages) Push(new { type = "msg", id = m.Id, role = m.Role, text = m.Text, image = m.Image is int n ? $"/img/{n}?t={_token}" : null });
...
function up(m){var e=els[m.id];if(!e){e=document.createElement('div');e.className='msg '+m.role;els[m.id]=e;feed.appendChild(e)}e.innerHTML='';e.appendChild(document.createTextNode(m.text||''));if(m.image){var im=document.createElement('img');im.src=m.image;e.appendChild(im)}feed.scrollTop=feed.scrollHeight}function heard(t){var h=q('#heard');h.innerHTML='<b>Heard: </b>';h.appendChild(document.createTextNode(t));h.classList.add('on');clearTimeout(heardTimer);heardTimer=setTimeout(()=>h.classList.remove('on'),12000)}
